...
function App () {
//useState
// const [data, setData] = useState("osibs") //we can store strings objects,array,booleans,int(numbers) inside this use state
// setTimeout (() => {
//   setData("This is me")
// }, 4000)
...
//onChange for input, for this, there has to be a state and a function to power the state, Thisis necesarry for theinput to work correctly
const handleUser = (e) => {
  setUsername(e.target.value) //updating the username

}
const handleEmail = (e) => {
  setEmail(e.target.value) //updating the email
}
const handleAge = (e) => {
...
//onSubmit
  ...
  return (
    ...
    <div>
      ...
      <input className="form-control" placeholder="username" id="username" onChange={handleUser} value={username}/>
      <input className="form-control" placeholder="Email" id="email" onChange={handleEmail}  value={email}/>
      <input className="form-control" placeholder="Age" id="age" onChange={handleAge} value={age}/>
      <button className="btn btn-dark" type="submit"> Click Me </button>
      </form> */}
    </div>
  )
}
...
// function App() {
  // return (
    //props is being used here
    // <div>
    //  { <SayName name='john' age = '23' /> }
    //  { <Colors color = 'red'/>}
    // </div>
    ...
    //    <Studentinfo info={data_} />

        {/* below is for outputting 'data' to the site */}
...
{/* or */}
        ...
        {/* {data.map((index) => ( */}
        {/* // <Students loadData = {index} key = {index.name} /> */}

        {/* //BELOW, WE HAVE A TENARY OPERATOR BECAUSE WE CAN NOT USE IF STATEMENTS IN JSX.
          //IT SIMPLY SAYS IF NEWDATA > 0, DISPLAY DATA BASE BY OUTPUTTING LIST, ELSE DISPLAY 'NO DATA'
          //WE CAN ALSO CREATE A SEPARATE COMPONENT FOR THE ELSE STATEMENT, EXAMPLE AFTER THIS 
          newData > 0 ? <Students loadData = {index} key = {index.name} /> : <h1> NO DATA </h1>
        ))} */}

        {/* this will show the error 'no data' 5 times because you put the tenery operator inside the map, the map is supposed to be a part of the operator instead */}
//       </div>
//     </div>
//   );
// }
...
//this below is for "data"
// function Students(props) {
//   return (
//     <div>
//       <div class="card m-4" style={{ width: "10rem" }}>
//         <img src={props.loadData.image} class="card-img-top" alt="..." height="300" />
//         <hr/>
//         <div class="card-body">
//           <h5 class="card-title">{props.loadData.name}</h5>
//           <hr></hr>
//           <p class="card-text">{props.loadData.words }</p>
...
//props
//props can be used to pass information from a mother function to a child function or vice versa
//you will see it in action on the app function
